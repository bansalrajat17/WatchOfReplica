 ...
 * @return string
 */
function slick_menu_icon_picker_get_icon_url( $type, $id, $size = 'thumbnail' ) {
	$url = '';
 ...
 * @return mixed
 */
function slick_menu_icon_picker_field( $args, $echo = true ) {
	$defaults = array(
		'id'    => '',
		'name'  => '',
		'value' => array(
		...
		'select' => sprintf( '<a class="ipf-select">%s</a>', esc_html__( 'Select Icon', 'slick-menu-icon-picker-field' ) ),
		'remove' => sprintf( '<a class="ipf-remove button hidden">%s</a>', esc_html__( 'Remove', 'slick-menu-icon-picker-field' ) ),
	);
	...
	$field .= sprintf(
		...
		esc_attr( slick_menu_icon_picker_get_icon_url( $args['value']['type'], $args['value']['icon'] ) )
	);
	$field .= '</div>';
