...
final class SM_Icon_Picker_Fontpack {

	/**
	 * SM_Icon_Picker_Fontpack singleton
	 *
	 * @static
	 * @since  0.1.0
	 * @access protected
	 * @var    SM_Icon_Picker_Fontpack
	 */
	 ...
	 * @static
	 * @since  0.1.0
	 * @return SM_Icon_Picker_Fontpack
	 */
	...
	protected function __construct() {
		 ...
		 */
		$this->dir = apply_filters( 'slick_menu_icon_picker_fontpacks_dir_path', WP_CONTENT_DIR . '/fontpacks' );
		 ...
		 */
		$this->url = apply_filters( 'slick_menu_icon_picker_fontpacks_dir_url', WP_CONTENT_URL . '/fontpacks' );

		$this->messages = array(
			'no_config'    => __( 'SM Icon Picker: %1$s was not found in %2$s.', 'slick-menu-icon-picker' ),
			'config_error' => __( 'SM Icon Picker: %s contains an error or more.', 'slick-menu-icon-picker' ),
			'invalid'      => __( 'SM Icon Picker: %1$s is not set or invalid in %2$s.', 'slick-menu-icon-picker' ),
			'duplicate'    => __( 'SM Icon Picker: %1$s is already registered. Please check your font pack config file: %2$s.', 'slick-menu-icon-picker' ),
		);
	...
	protected function register_packs() {
		...
		require_once "{$slick_menu_icon_picker->dir}/includes/types/fontello.php";

		foreach ( $this->packs as $pack_data ) {
			$slick_menu_icon_picker->registry->add( new SM_Icon_Picker_Type_Fontello( $pack_data ) );
		}
	}
	...
	protected function get_pack_data( DirectoryIterator $pack_dir ) {
		...
		$cache_id      = "slick_menu_icon_picker_fontpack_{$pack_dirname}";
		...
		$pack_data = array(
			'id'             => "pack-{$config['name']}",
			'name'           => sprintf( __( 'Pack: %s', 'slick-menu-icon-picker' ), $config['name'] ),
