...
class SM_RW_Meta_Box
{
	...
	public function __construct( $meta_box )
	{
		...
		$show = apply_filters( 'sm_rwmb_show', true, $this->meta_box );
		...
		{
			SM_RWMB_Field::call( $field, 'add_actions' );
		}
	...
	public function enqueue()
	{
		...
		wp_enqueue_style( 'rwmb', SM_RWMB_CSS_URL . 'style.css', array(), SM_RWMB_VER );
		if ( is_rtl() )
			wp_enqueue_style( 'sm-rwmb-rtl', SM_RWMB_CSS_URL . 'style-rtl.css', array(), SM_RWMB_VER );
		...
		foreach ( $fields as $field )
		{
			if ( $field['clone'] )
			{
				wp_enqueue_script( 'sm-rwmb-clone', SM_RWMB_JS_URL . 'clone.js', array( 'jquery-ui-sortable' ), SM_RWMB_VER, true );
				break;
			}
		}
		...
		foreach ( $fields as $field )
		{
			SM_RWMB_Field::call( $field, 'admin_enqueue_scripts' );
		}

		// Auto save
		if ( $this->meta_box['autosave'] )
			wp_enqueue_script( 'sm-rwmb-autosave', SM_RWMB_JS_URL . 'autosave.js', array( 'jquery' ), SM_RWMB_VER, true );

		/**
		 * Allow developers to enqueue more scripts and styles
		 * @param SM_RW_Meta_Box $object Meta Box object
		 */
		do_action( 'sm_rwmb_enqueue_scripts', $this );
	}
	...
	public function show()
	{
		...
		// Container
		printf(
			'<div class="sm-rwmb-meta-box" data-autosave="%s">',
			$this->meta_box['autosave'] ? 'true' : 'false'
		);

		wp_nonce_field( "sm-rwmb-save-{$this->meta_box['id']}", "nonce_{$this->meta_box['id']}" );

		// Allow users to add custom code before meta box content
		// 1st action applies to all meta boxes
		// 2nd action applies to only current meta box
		do_action( 'sm_rwmb_before', $this );
		do_action( "sm_rwmb_before_{$this->meta_box['id']}", $this );

		foreach ( $this->fields as $field )
		{
			SM_RWMB_Field::call( 'show', $field, $saved );
		}

		// Allow users to add custom code after meta box content
		// 1st action applies to all meta boxes
		// 2nd action applies to only current meta box
		do_action( 'sm_rwmb_after', $this );
		do_action( "sm_rwmb_after_{$this->meta_box['id']}", $this );
	...
	public function save_post( $post_id )
	{
		...
		// Before save action
		do_action( 'sm_rwmb_before_save_post', $post_id );
		do_action( "sm_rwmb_{$this->meta_box['id']}_before_save_post", $post_id );
		...
		foreach ( $this->fields as $field )
		{
			...
			$new = SM_RWMB_Field::call( $field, 'value', $new, $old, $post_id );
			$new = SM_RWMB_Field::filter( 'value', $new, $field, $old );

			// Call defined method to save meta value, if there's no methods, call common one
			SM_RWMB_Field::call( $field, 'save', $new, $old, $post_id );
		}

		// After save action
		do_action( 'sm_rwmb_after_save_post', $post_id );
		do_action( "sm_rwmb_{$this->meta_box['id']}_after_save_post", $post_id );
	}
	...
	protected function validate()
	{
		...
		return
			...
			&& wp_verify_nonce( $nonce, "sm-rwmb-save-{$this->meta_box['id']}" );
	}
	...
	public static function normalize( $meta_box )
	{
		...
		$meta_box = apply_filters( 'sm_rwmb_normalize_meta_box', $meta_box );
		$meta_box = apply_filters( "sm_rwmb_normalize_{$meta_box['id']}_meta_box", $meta_box );
	...
	public static function normalize_fields( $fields )
	{
		...
		{
			$field = SM_RWMB_Field::call( 'normalize', $field );

			if ( isset( $field['fields'] ) )
				$field['fields'] = self::normalize_fields( $field['fields'] );

			// Allow to add default values for fields
			$field = apply_filters( 'sm_rwmb_normalize_field', $field );
			$field = apply_filters( "sm_rwmb_normalize_{$field['type']}_field", $field );
			$field = apply_filters( "sm_rwmb_normalize_{$field['id']}_field", $field );
	...
	{
		return SM_RWMB_Field::get_class_name( $field );
	}
