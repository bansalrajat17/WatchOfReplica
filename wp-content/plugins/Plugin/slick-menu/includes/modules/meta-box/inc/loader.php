...
class SM_RWMB_Loader
{
	...
	public function constants()
	{
		...
		define( 'SM_RWMB_VER', '4.8.7' );

		list( $path, $url ) = self::get_path();

		// Plugin URLs, for fast enqueuing scripts and styles
		define( 'SM_RWMB_URL', $url );
		define( 'SM_RWMB_JS_URL', trailingslashit( SM_RWMB_URL . 'js' ) );
		define( 'SM_RWMB_CSS_URL', trailingslashit( SM_RWMB_URL . 'css' ) );

		// Plugin paths, for including files
		define( 'SM_RWMB_DIR', $path );
		define( 'SM_RWMB_INC_DIR', trailingslashit( SM_RWMB_DIR . 'inc' ) );
		define( 'SM_RWMB_FIELDS_DIR', trailingslashit( SM_RWMB_INC_DIR . 'fields' ) );
	}
	...
	public function autoload( $class )
	{
		...
		if ( 'SM_RW_Meta_Box' != $class && 0 !== strpos( $class, 'SM_RWMB_' ) )
		{
			return;
		}
		...
		// Get file name
		$file = 'meta-box';
		if ( 'SM_RW_Meta_Box' != $class )
		{
			// Remove prefix 'SM_RWMB_'
			$file = substr( $class, 8 );
		...
		$dirs = array( SM_RWMB_INC_DIR, SM_RWMB_FIELDS_DIR, trailingslashit( SM_RWMB_INC_DIR . 'walkers' ) );
		foreach ( $dirs as $dir )
		{
	...
	public function autoload_fallback()
	{
		...
		{
			require SM_RWMB_INC_DIR . "$file.php";
		}
	}
	...
	public function init()
	{
		...
		{
			// Validation module
			new SM_RWMB_Validation;
		}

		// Public functions
		require SM_RWMB_INC_DIR . 'functions.php';
	}
}
