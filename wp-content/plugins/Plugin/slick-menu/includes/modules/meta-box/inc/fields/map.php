 ...
 * Map field class.
 */
class SM_RWMB_Map_Field extends SM_RWMB_Field
{
	/**
	...
	static function admin_enqueue_scripts()
	{
		 ...
		 */
		$google_maps_url = apply_filters( 'sm_rwmb_google_maps_url', 'https://maps.google.com/maps/api/js?sensor=false' );
		wp_register_script( 'google-maps', esc_url_raw( $google_maps_url ), array(), '', true );
		wp_enqueue_style( 'sm-rwmb-map', SM_RWMB_CSS_URL . 'map.css' );
		wp_enqueue_script( 'sm-rwmb-map', SM_RWMB_JS_URL . 'map.js', array( 'jquery-ui-autocomplete', 'google-maps' ), SM_RWMB_VER, true );
	}
	...
	{
		$html = '<div class="sm-rwmb-map-field">';

		$html .= sprintf(
			'<div class="sm-rwmb-map-canvas" data-default-loc="%s"></div>
			<input type="hidden" name="%s" class="sm-rwmb-map-coordinate" value="%s">',
			esc_attr( $field['std'] ),
		...
		{
			$html .= sprintf(
				'<button class="button sm-rwmb-map-goto-address-button" value="%s">%s</button>',
	...
	static function the_value( $field, $args = array(), $post_id = null )
	{
		...
		$google_maps_url = apply_filters( 'sm_rwmb_google_maps_url', 'https://maps.google.com/maps/api/js?sensor=false' );
		wp_register_script( 'google-maps', esc_url_raw( $google_maps_url ), array(), '', true );
		wp_enqueue_script( 'sm-rwmb-map-frontend', SM_RWMB_JS_URL . 'map-frontend.js', array( 'google-maps' ), '', true );
		...
		$output = sprintf(
			'<div class="sm-rwmb-map-canvas" data-map_options="%s" style="width:%s;height:%s"></div>',
