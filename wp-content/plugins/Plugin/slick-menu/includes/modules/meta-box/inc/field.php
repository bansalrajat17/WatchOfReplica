...
abstract class SM_RWMB_Field
{
	...
	public static function show( $field, $saved )
	{
		...
		if ( $field['clone'] )
		{
			...
			foreach ( $meta as $index => $sub_meta )
			{
				...
				// Wrap field HTML in a div with class="sm-rwmb-clone" if needed
				$class     = "sm-rwmb-clone sm-rwmb-{$field['type']}-clone";
				$sort_icon = '';
				if ( $field['sort_clone'] )
				{
					$class .= ' sm-rwmb-sort-clone';
					$sort_icon = "<a href='javascript:;' class='sm-rwmb-clone-icon'></a>";
				}
		...
		$classes = "sm-rwmb-field sm-rwmb-{$field['type']}-wrapper " . $field['class'];
	...
	public static function begin_html( $meta, $field )
	{
		...
		{
			$field_label = sprintf(
				'<div class="sm-rwmb-label"><label for="%s">%s</label></div>',
				$field['id'],
				$field['name']
			);
		}
	...
	{
		$text = apply_filters( 'sm_rwmb_add_clone_button_text', __( '+ Add more', 'meta-box' ), $field );
	...
	public static function remove_clone_button( $field )
	{
		...
		$text = apply_filters( 'sm_rwmb_remove_clone_button_text', $icon, $field );
		return "<a href='#' class='sm-rwmb-button remove-clone'>$text</a>";
	}
	 ...
	 * sm_rwmb_the_field function later.
	 *
	 * @use self::get_value()
	 * @see sm_rwmb_the_value()
	 *
	 * @param  array    $field   Field parameters
	 * @param  array    $args    Additional arguments. Rarely used. See specific fields for details
	 * @param  int|null $post_id Post ID. null for current post. Optional.
	...
	public static function get_class_name( $field )
	{
		...
		$class = 'SM_RWMB_' . ucwords( $type ) . '_Field';
		$class = str_replace( ' ', '_', $class );
		return class_exists( $class ) ? $class : 'SM_RWMB_Input_Field';
	}
	 ...
	 * Filters:
	 * - sm_rwmb_{$name}
	 * - sm_rwmb_{$field['type']}_{$name}
	 * - sm_rwmb_{$field['id']}_{$name}
	 * @return mixed
	 */
	public static function filter()
	{
		...
		if ( isset( $field['id'] ) )
		{
			$filters[] = 'sm_rwmb_' . $field['id'] . '_' . $name;
		}
