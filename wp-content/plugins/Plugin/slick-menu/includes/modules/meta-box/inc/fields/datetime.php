 ...
 */
class SM_RWMB_Datetime_Field extends SM_RWMB_Text_Field
{
	/**
	...
	public static function admin_register_scripts()
	{
		...
		wp_register_style( 'wp-datepicker', SM_RWMB_CSS_URL . 'datepicker.css', array( 'jquery-ui-core', 'jquery-ui-theme' ), '1.8.17' );
		...
		wp_register_script( 'sm-rwmb-datetime', SM_RWMB_JS_URL . 'datetime.js', array( 'jquery-ui-datepicker-i18n', 'jquery-ui-timepicker-i18n' ), SM_RWMB_VER, true );
		wp_register_script( 'sm-rwmb-date', SM_RWMB_JS_URL . 'date.js', array( 'jquery-ui-datepicker-i18n', 'jquery-ui-timepicker-i18n' ), SM_RWMB_VER, true );
		wp_register_script( 'sm-rwmb-time', SM_RWMB_JS_URL . 'time.js', array( 'jquery-ui-timepicker-i18n' ), SM_RWMB_VER, true );
		...
		{
			if ( ! $wp_scripts->get_data( "sm-rwmb-$handle", 'data' ) )
			{
				wp_localize_script( "sm-rwmb-$handle", 'SM_RWMB_' . ucfirst( $handle ), $data );
			}
		}
	}
	...
	public static function admin_enqueue_scripts()
	{
		...
		wp_enqueue_script( 'sm-rwmb-datetime' );
	}
	...
	public static function html( $meta, $field )
	{
		...
		if ( $field['timestamp'] )
		{
			...
			$output .= sprintf(
				'<input type="hidden" name="%s" class="sm-rwmb-datetime-timestamp" value="%s">',
		...
		if ( $field['inline'] )
		{
			$output .= '<div class="sm-rwmb-datetime-inline"></div>';
		}
	...
	public static function normalize( $field )
	{
		...
		$field = SM_RWMB_Text_Field::normalize( $field );
