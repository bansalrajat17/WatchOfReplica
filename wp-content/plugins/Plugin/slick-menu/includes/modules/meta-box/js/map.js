(function ( $ )
{
	...
	MapField.prototype = {
		...
		{
			this.canvas = this.$container.find( '.sm-rwmb-map-canvas' )[0];
			this.$coordinate = this.$container.find( '.sm-rwmb-map-coordinate' );
			this.$findButton = this.$container.find( '.sm-rwmb-map-goto-address-button' );
		...
		addListeners      : function ()
		{
			 ...
			 *      ('resize' Event)
			 */
			$( window ).on( 'sm_rwmb_map_refresh', function()
			{
				that.refreshMap();
			} );
	...
	$( function ()
	{
		...
		$( '.sm-rwmb-input' ).on( 'clone', function ()
		{
			$( '.sm-rwmb-map-field' ).each( function ()
			{
				var field = new MapField( $( this ) );
