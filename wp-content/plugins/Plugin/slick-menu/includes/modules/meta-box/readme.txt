...
- [MB Admin Columns](https://metabox.io/plugins/sm-mb-admin-columns/): Display custom fields in table columns in admin screens for All Posts (types).
- [MB Term Meta](https://metabox.io/plugins/sm-mb-term-meta/): Add meta data to categories, tags or any custom taxonomy with simple syntax.
- [MB Settings Page](https://metabox.io/plugins/sm-mb-settings-page/): Create settings pages for themes, plugins or websites with beautiful syntax.
- [MB Custom Post Type](https://wordpress.org/plugins/sm-mb-custom-post-type/): Create and manage custom post types and taxonomies easily in WordPress with an easy-to-use interface.
...
* Improvement: Add support for custom marker for map field (param `marker_icon`) in sm_rwmb_meta function
* Improvement: Add `limit` option for media fields in `sm_rwmb_meta` function when retrieving meta value.
* Improvement: Add `sm_rwmb_option_label` filter for choice fields (user, post, taxonomy) so users can choose which object field is used as label
...
* Fix: `sm_rwmb_meta` now can display rich content from `oembed` field
...
* Fix: Notice for missing parameters for `sm_rwmb_meta` field for `map`
...
* Fix: sm_rwmb_meta works incorrectly for image fields if multiple=false
...
* Improvement: Add `sm_rwmb_get_field` and `sm_rwmb_the_field` functions to get and display field values in the frontend
...
* Improvement: No errors showing if invalid value is returned from `sm_rwmb_meta_boxes` filter
...
* Fix: Don't return non-existing files or images via sm_rwmb_meta function
...
* Improvement: allow to pass more arguments to "get_terms" function when getting meta value with "sm_rwmb_meta"
...
* Improvement: add more params to "sm_rwmb_meta" filter
...
* Improvement: add `ID` to results returned by `sm_rwmb_meta` when getting meta value of file & image
...
* Improvement: add `sm-rwmb-{$field_type}-wrapper` class to field markup
* Improvement: Add [sm_rwmb_meta meta_key="..."] shortcode. Attributes are the same as `sm_rwmb_meta` function.
* Code refactored
...
* Taxonomy field now supported by `sm_rwmb_meta` function
