...
if ( class_exists( 'SM_RWMB_Field' ) )
{
	...
	class SM_RWMB_Image_Bg_Select_Field extends SM_RWMB_Field
	{
		/**
		...
		{
			$html = array();
			$tpl  = '<label class="sm-rwmb-image-select" style="background-image:url(%s)" title="%s"><img src="%s" style="opacity:0;visibility: hidden;"><input type="%s" class="sm-rwmb-image_select hidden" name="%s" value="%s"%s></label>';
		...
		{
			return sprintf( '<img src="%s" style="opacity:0;visibility: hidden;">', esc_url( $field['options'][$value] ) );
		}
	}
}